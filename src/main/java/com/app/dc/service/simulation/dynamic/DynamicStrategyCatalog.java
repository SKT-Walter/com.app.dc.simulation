...
@Service public class DynamicStrategyCatalog {
 ...
 @PostConstruct public void load(){try{String json=new String(Files.readAllBytes(Paths.get(file)),StandardCharsets.UTF_8);Catalog c=JsonUtils.Deserialize(json,Catalog.class);if(c==null||c.strategies==null||c.strategies.isEmpty())throw new IllegalStateException("empty catalog");strategies=c.strategies;}catch(Exception e){throw new IllegalStateException("cannot load dynamic strategy catalog: "+file,e);}}

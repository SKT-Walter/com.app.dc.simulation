...
/** ETH-only slow-structure pullback continuation strategy. */
@Service("ethStructuralBullTrend")
public class EthStructuralBullTrendBacktestStrategy extends AbstractTrendBacktestStrategy {
    @Autowired private EthStructuralBullTrendService lifecycle;
    public String getName(){return EthStructuralBullTrendService.STRATEGY;}
    public Signal evaluate(String symbol,String text,BarSeries series,TTbookOhlc current){
        ...
        if(s.actionable&&s.barIndex==series.getEndIndex()&&Double.isFinite(s.stopPrice)){
            ...
            signal.takerPrice=null;signal.algoName="ETH_STRUCTURAL_BULL";
